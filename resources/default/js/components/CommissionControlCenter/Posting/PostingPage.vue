        ...
        <div class="m-portlet__body">
            ...
            <div v-if="totalData.length" class="row" style="margin-top: 25px;">
                <div class="col-12 text-center">
                    ...
                    <button v-if="commission === 'fsb'" type="button" class="btn btn-info" @click="onExportClick()" style="min-width: 88px;margin-bottom: 7px;">Export</button>
                    <BallLoader v-if="isPostProcessing" />
                    <button v-else type="button" class="btn btn-danger" @click="onPostBtnClick(null)" style="min-width: 88px;margin-bottom: 7px;">Post</button>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onPostBtnClick(verificationCode) {
                const self = this;
                ...
                $.ajax({
                    type: 'POST',
                    url: baseUrl + '/commission-control-center/post-commission',
                    data: JSON.stringify({
                        commission: self.commission,
            ...
            onExportClick() {
                var f = $('#summaryDetails').dataTable().fnSettings();
                var q = $('#summaryDetails_filter input').val();
                var i = f.aaSorting[0][0];
                var o = f.aaSorting[0][1];

                var u = "/exp-commission-detail/" + (f.aoColumns[i].data ? f.aoColumns[i].data : 'distid') + "/" + o + "/" + q + "/?total=";
