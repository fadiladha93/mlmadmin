                ...
                <table class="table m-table m-table--head-separator-info">
                    ...
                    @foreach($commissions as $c)
                        <tr>
                            ...
                            <td>{{$c->percent * 100}}% Unilevel Commission (Level {{ $c->level }}) for product (sku: {{ $c->order->product->sku }}) purchased with order ORD{{ $c->order->order->id }} on {{ $c->order->order->created_date }}</td>
                            <td>Level {{$c->level}}</td>
                            <td>${{$c->amount}}</td>
                            {{--<td>
                                <a href="#" class="unilevel-details" data-toggle="modal" data-target="#orderDetails" data-id="{{ $c->order_id }}" data-date="{{ $c->end_date }}">
