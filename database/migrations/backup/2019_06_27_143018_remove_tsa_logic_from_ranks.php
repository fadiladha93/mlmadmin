...
class RemoveTsaLogicFromRanks extends Migration
{
    /**
     * Run the migrations.
     *
...
AS $BODY$DECLARE 
             ...
             cur_transacs CURSOR (d1 date,d2 date)
                FOR SELECT * FROM vorder_product_qv where date(created_dt) between d1 and d2;
            
             cur_all_users CURSOR
                FOR SELECT id,distid,COALESCE(current_month_qv, 0) as current_month_qv,COALESCE(current_month_tsa, 0) as current_month_tsa,COALESCE(current_month_pqv, 0) as current_month_pqv, COALESCE(current_month_cv, 0) as current_month_cv,is_active FROM users ;
               ...
               LOOP
                    ...
                    LOOP
                        ...
                        IF crank.rankval IS NOT NULL THEN
                            ...
                            --Data already available for a particular user for particular month, so update				
                                UPDATE public.user_rank_history 
                                SET monthly_rank=crank.rankval,monthly_rank_desc=crank.rankdesc,monthly_qv= curr_month_qv,qualified_qv=crank.rank_qv,qualified_tsa=round(coalesce(crank2.rank_tsa,0),0),monthly_tsa=curr_month_tsa,monthly_cv = curr_month_cv
                                WHERE user_id=cuser.id and period=enddate_of_month;		
                            else
                            --Data unavailable for a particular user for particular month, so insert
                                INSERT INTO public.user_rank_history(user_id, monthly_rank,monthly_rank_desc, period, monthly_qv,qualified_qv,qualified_tsa,monthly_tsa, monthly_cv) 
                                VALUES (cuser.id, crank.rankval,crank.rankdesc,enddate_of_month, curr_month_qv,crank.rank_qv,round(coalesce(crank2.rank_tsa,0),0),curr_month_tsa, curr_month_cv);
                            end if;	
