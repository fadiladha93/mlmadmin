...
class FixCalculateFsbApproveFunction extends Migration
{
    /**
     * Run the migrations.
     *
...
DECLARE 
 ...
 BEGIN
	payment_date := NOW()::timestamp(0);
	SELECT processed_date into commission_date from commission_temp_post LIMIT 1;
   ...
   LOOP
	  ...
	  VALUES (commis.user_id, user_ob, user_ob+commis.amount, commis.amount, \'DEPOSIT\',payment_date,0,\'FSB\');
	  ...
	  WHERE id=commis.user_id;		
		...
		VALUES (commission_date, 1, \'PAID\',commis.amount ,commis.user_id);

		-- insert to get the total per user, if no record already available, insert, otherwise update
		select count(*) into week_sum_rec from week_summary where user_id=commis.user_id and week_ending=commission_date;
		if week_sum_rec>0 then
				update "week_summary" set total=total+commis.amount where user_id=commis.user_id and week_ending=commission_date;
		else
				INSERT INTO "week_summary" (week_ending, total, user_id) 
				VALUES (commission_date, commis.amount, commis.user_id);
		end if;		
