...
class RollbackTimeZoneQvCalculateFunction extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement('DROP FUNCTION IF EXISTS calculate_downline_qv_with_tsa(timestamp, timestamp, timestamp);');
...
AS $BODY$DECLARE 
    ...
    {
        DB::statement('DROP FUNCTION IF EXISTS calculate_downline_qv_with_tsa(timestamp, timestamp);');
...
AS $BODY$DECLARE 
             ...
             BEGIN
               ...
               -- first date of month
               SELECT date_trunc(\'month\', now_utc)::DATE into startdate_of_month;
               -- last date of month
               SELECT (date_trunc(\'month\', now_utc)::DATE + interval \'1 month - 1 day\')::DATE into enddate_of_month;
               ...
               ----update users set current_month_pqv=su.qv
               ----from (SELECT userid,sum(qv) as qv FROM vorder_product_qv 
               ----      where date(created_dt) between date_trunc(\'day\', now_utc)::DATE - interval \'30 days\' and date_trunc(\'day\', now_utc)::DATE + interval \' + 1 day - 1 second\' group by userid) su
               ----where users.id=su.userid;
               ...
               LOOP
                    ...
                    LOOP
                        ...
                        IF crank.rankval IS NOT NULL THEN
                            ...
                            IF rank_exists < 1 THEN
                                INSERT INTO rank_history(users_id, lifetime_rank, created_dt, remarks)
								VALUES (cuser.id, crank.rankval, now_utc, rank_remarks);
                            END IF;
                    ...
                    SELECT count(*) FROM rank_history INTO rank_exists WHERE users_id=cuser.id;
                    IF rank_exists < 1 THEN
                        INSERT INTO rank_history(users_id, lifetime_rank, created_dt) VALUES (cuser.id, 10, now_utc);
                    END IF;
               END LOOP;
               -- Close the cursor all_users
               CLOSE cur_all_users;      
               raise info \'Ended %\', now_utc;
               
               --insert a record that this process ran on certain date and time
               truncate rank_log;
               insert into rank_log (worked_on) values (now_utc);   
            END;$BODY$;
        ');
    }
}
