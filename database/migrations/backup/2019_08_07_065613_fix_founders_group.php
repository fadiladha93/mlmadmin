...
class FixFoundersGroup extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement('UPDATE users SET founder = NULL;');
        ...
        DB::statement('
            ...
            ON o.id = oi.orderid
            WHERE u.id = o.userid 
            AND u.current_product_id IN (4, 13, 16)
            AND u.id NOT IN (
                SELECT u.id
                FROM users u
                ...
                WHERE oi.productid IN (4, 7, 9, 10, 13, 16, 17, 18, 19, 20) AND oi.created_dt >= \'2019-08-02 00:00:00\'
            );
        ');
...
begin
	...
	SELECT created_dt INTO enrollment_date FROM users WHERE id = user_id LIMIT 1;
	SELECT founder INTO is_founder FROM users WHERE id = user_id LIMIT 1;
	    
	IF is_founder AND enrollment_date < (enrollment_date + interval \'1 year\') THEN
	    pack_percent := 0.2;
	END IF;
...
$BODY$;
    ...
    public function down()
    {
        ...
        DB::statement('
            ...
            JOIN "orderItem" oi
            ON o.id = oi.orderid
            WHERE u.id = o.userid AND oi.productid IN (4, 7, 9, 10, 13, 16, 17, 18, 19, 20)  AND oi.created_dt <= \'2019-08-01 23:59:59\';
        ');
