...
class FixTimeZoneInRankCalculateFunction extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement('DROP FUNCTION IF EXISTS calculate_downline_qv_with_tsa();');
        DB::statement('DROP FUNCTION IF EXISTS check_binary_verification(integer, integer);');
        DB::statement('DROP FUNCTION IF EXISTS get_rank_by_percentage(character varying, bigint, numeric, bigint, numeric, numeric);');
...
AS $BODY$
     SELECT count(u.id) FROM users u
        ...
        WHERE (
               u.id IN (
                    SELECT userid FROM orders
                    WHERE created_dt >= from_date AND created_dt <= to_date GROUP BY userid HAVING sum(orderqv) >= 100
                    )
                    or (u.current_product_id = 16 and u.created_dt >= date(to_date) - interval \'1 year\')
...
END;$BODY$;
    ...
    public function down()
    {
        ...
        DB::statement('DROP FUNCTION IF EXISTS check_binary_verification(integer, integer, timestamp, timestamp);');
					 ...
					 select u.id from users u 
            			...
            			on bp.user_id = u.id
           				where (
                    		u.id in (
                      			select userid from orders
                      			where date(created_dt) >= NOW()::date - interval \'1 month\' group by userid having sum(orderqv) >= 100
                      		)
                    	or (u.current_product_id = 16 and u.created_dt >= NOW()::date - interval \'1 year\')
                    	or u.distid in (
							\'A1357703\',
							\'A1637504\',
							\'TSA9846698\',
