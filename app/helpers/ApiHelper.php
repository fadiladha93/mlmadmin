...
class ApiHelper
{
    ...
    protected static function buildBillingEndpoint(string $endpoint): string
    {
        return 'https://' . env('BILLING_BASE_URL') . $endpoint;
    }


    public static function billingRequest(string $method, string $endpoint, array $form_params, array $custom_headers = [])
    {
        $self = new self();

        $url = self::buildBillingEndpoint($endpoint);
