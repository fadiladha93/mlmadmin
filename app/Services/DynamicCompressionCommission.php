...
abstract class DynamicCompressionCommission
{
    ...
    protected function getOrderItems($userId, $dateFrom, $dateTo)
    {
        return DB::table('orders')
            ...
            ->where('userid', $userId)
            //->whereIn('products.producttype', [static::SUBSCRIPTION_ID]) This is the old code
            ->whereIn('products.producttype', [static::SUBSCRIPTION_ID, static::SIMPLE_PRODUCT, static::BUUMERANG])
    ...
    protected function addCommissionRow($user, $level, $orderId, $amount)
    {
        DB::table(static::TABLE_NAME)->insert(
            [
                ...
                'status' => UnilevelCommission::CALCULATED_STATUS,
            ]
        );
    }
