...
class AchievedRankService
{
    ...
    public function calculateRank($startDate, $endDate)
    {
        ...
        //Update rank tracker table - lock ewallet screen
        $rank_tracker = RankTracker::find(1);
        $rank_tracker->is_running = true;
        $rank_tracker->last_run_start = Carbon::now();
        $rank_tracker->save();
        ...
        (new BuumLogger('Rank Calculation Completed', BuumLogger::LOG_TYPE_INFO))->toSlackAndLog();

        //Update rank tracker table - unlock ewallet screen
        $rank_tracker = RankTracker::find(1);
        $rank_tracker->is_running = false;
        $rank_tracker->last_run_finish = Carbon::now();
        $rank_tracker->save();
    }
    ...
    public function calculateQCRank()
    {
        ...
        User::where('current_month_rank', '>=', RankInterface::RANK_VALUE_EXECUTIVE)
            ->orderBy('level', 'DESC')
            ->chunk(static::BATCH_SIZE, function ($users, $page) use ($numBatches, $numUsers, $showEveryThisAmount, &$counter) {
                ...
                foreach ($users as $user) {
                    ...
                    if ($counter == 1 || $counter == $numUsers || ($counter % $showEveryThisAmount) == 0) {
                        ...
                        $message = "Current ranks calculation user progress: $userProgress";
                        (new BuumLogger($message, BuumLogger::LOG_TYPE_INFO))->toSlackAndLog();
                    }
