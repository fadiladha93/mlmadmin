...
class TsbCommissionService
{
    ...
    public function calculateCommission($filename, Carbon $endDate)
    {
        ...
        foreach ($results as $result) {
            ...
            if (Order::orderWithTransactionIdExists($transactionId)) {
                Log::warning('TSB already done? Order for user id already done',
    ...
    private function processCommission($parsedResult, $transactionId, Carbon $endDate)
    {
        ...
        $orderId = \App\Order::addNew(
            $user->id,
            (float)$parsedResult['total'],
            (float)$parsedResult['total'],
            0,
            (int)$parsedResult['total'],
            (int)$parsedResult['cv'],
            $transactionId,
            null,
            null,
        ...
        DB::table('orderItem')->insert([
            ...
            'quantity' => 1,
            'itemprice' => (float)$parsedResult['total'],
            'bv' => 0,
            'cv' => (int)$parsedResult['cv'],
            'qv' => (int)$parsedResult['total'],
    ...
    public function payoutCommission($startDate, $endDate)
    {
        ...
        foreach ($commissions as $commission) {
            ...
            $transactionId = EwalletTransaction::addCommission(
                    $commission->user_id,
                    $commission->amount,
                    EwalletTransaction::TYPE_TSB_COMMISSION,
                    "TSB Bonus"
            );
