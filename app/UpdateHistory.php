...
class UpdateHistory extends Model
{
    ...
    const TYPE_BOOMERANG_INV = "BOOMERANG_INV";
    ...
    public static function productAdd($productId)
    {
        ...
        $after['num_boomerangs'] = $prod_after->num_boomerangs;
        $after['sponsor_boomerangs'] = $prod_after->sponsor_boomerangs;
    ...
    public static function productUpdate($productId, $rec, $req)
    {
        ...
        if ($rec->num_boomerangs != $req->num_boomerangs) {
            $before['Number of Boomerangs'] = $rec->num_boomerangs;
            $after['Number of Boomerangs'] = $req->num_boomerangs;
        }
        if ($rec->sponsor_boomerangs != $req->sponsor_boomerangs) {
            $before['Sponsor Boomerangs'] = $rec->sponsor_boomerangs;
            $after['Sponsor Boomerangs'] = $req->sponsor_boomerangs;
        }

        if ($rec->shipping_price != $req->shipping_price) {
            $before['Shipping Price'] = $rec->shipping_price;
            $after['Shipping Price'] = $req->shipping_price;
        }

        if ($rec->shipping_enabled != $req->shipping_enabled) {
            $before['Shopping Enabled'] = $rec->shipping_enabled;
            $after['Shopping Enabled'] = $req->shipping_enabled;
        }

        if ($rec->allow_multiple_on_cart != $req->allow_multiple_on_cart) {
            $before['Allow Multiple on Cart'] = $rec->allow_multiple_on_cart;
            $after['Allow Multiple on Cart'] = $req->allow_multiple_on_cart;
        }

        if ($rec->tax_code != $req->tax_code) {
            $before['Tax Code'] = $rec->tax_code;
            $after['Tax Code'] = $req->tax_code;
        }

        if ($rec->is_taxable != $req->is_taxable) {
            $before['Taxable'] = $rec->is_taxable;
            $after['Taxable'] = $req->is_taxable;
        }
    ...
    public static function boomerangInvAdd($boomerangInvId)
    {
        $before = array();
        $after = array();

        $boomerang_after = BoomerangInv::getById($boomerangInvId);
        $after['User ID'] = $boomerang_after->userid;
        $after['Pending Total'] = $boomerang_after->pending_tot;
        $after['Available Total'] = $boomerang_after->available_tot;
        self::addNew(self::TYPE_BOOMERANG_INV, $boomerangInvId, $before, $after, self::MODE_ADD);
    }

    public static function boomerangInvUpdate($boomerangInvId, $rec, $req)
    {
        $before = [];
        $after  = [];
        //
        if ($rec->available_tot != ($req->new_boomerang - $rec->pending_tot)) {
            $before['Available Total'] = $rec->available_tot;
            $after['Available Total'] = $req->new_boomerang - $rec->pending_tot;
        }

        self::addNew(self::TYPE_BOOMERANG_INV, $boomerangInvId, $before, $after);
    }

    public static function boomerangMaxAvailableInvUpdate($boomerangInvId, $rec, $req)
    {
        $before['Max Available'] = $rec->max_available;
        $after['Max Available'] = $req->max_available;
