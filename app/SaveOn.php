...
class SaveOn extends Model {
    ...
    public static function getMembersInformation($contractNumber) {
        ...
        $saveOnAPI = new \SOR(\App\Product::ID_NCREASE_ISBO);
        $postData = array(
    ...
    public static function enableUser($productId, $contractNumber, $note) {
        ...
        $user = \App\User::getByDistId($contractNumber);
        $logId = \App\Helper::logApiRequests($user->id, 'SOR - enable user', $endPoint, $postData);
    ...
    public static function disableUser($productId, $contractNumber, $note) {
        ...
        $user = \App\User::getByDistId($contractNumber);
        $logId = \App\Helper::logApiRequests($user->id, 'SOR - disable user', $endPoint, $postData);
        \App\Helper::logApiResponse($logId->id, $responseBody);
    ...
    public static function SORCreateUser($userId, $productId, $userAddress) {
        ...
        $logId = \App\Helper::logApiRequests($user->id, 'SOR - create user', $endPoint, $postData);
        //create user
        $saveOnAPI = new \SOR($productId);
        try {
            $jsonBody = $saveOnAPI->_post($endPoint, $postData, true);
    ...
    /** FOR CUSTOMERS, START * */
    public static function SORCreateUserWithToken_customers($firstName, $lastName, $email, $mobile, $password, $boomerangCode) {
        $endPoint = config('api_endpoints.SORCreateUser');
        $postData = array(
            'Email' => $email,
            'Password' => $password,
