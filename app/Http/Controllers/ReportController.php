...
use App\Services\UnilevelCommission as UnilevelService;
use App\User;
...
class ReportController extends Controller
{
    ...
    public function __construct(BinaryPlanService $binaryPlanService)
    {
        ...
        $this->middleware('auth.admin', ['except' => [
            ...
            'tools',
            'commissionReport',
            'commissionReportWeekly',
            'commissionReportWeeklyDetails',
            'personallyEnrolledReport',
            'getPersonallyEnrolledByPackage',
            'distributorsByLevel',
            'distributorsByLevelDetail',
            'getDistributorsByLevelDetailDataTable',
            'getOrgDrillDownDataTable',
            'personallyEnrolledByPackage',
            'orgDrillDown',
            'invoice',
            'viewInvoice',
            'downloadInvoice',
            'entireOrganizationReport',
            'getEnrolledInternReportDataTable',
            'unilevelCommissionDetails',
            'getEntireOrganizationReportDataTable',
            'leadershipCommissionDetails',
            'tsbCommissionDetails',
            'orderDetails',
            'weeklyEnrollmentReport',
            'weeklyEnrollmentReportDataTable',
            'weeklyBinaryReport',
            'weeklyBinaryReportDataTable',
            'getPearReport',
            'getPearReportByUser',
            'getHistoricalReport',
            'getLosReport'
        ...
        $this->middleware('auth.export_report', [
            'only' => [
                'exportPersonallyEnrolledDistributors',
                'exportVipDistData',
                'exportDistributorByRank',
                'exportDistributorByRankDetail',
                'exportSalesByPaymentMethod',
                'exportIDecideOrSor',
                'exportSubscriptionReport',
                'exportSubscriptionByPaymentMethod',
                'exportSapphiresByCountry',
                'exportDiamondByCountry',
                'exportMonthlyEarning',
                'exportMonthlyTopRecruiters',
                'exportMonthlyTopCustomers',
                'exportRankAdvancement',
...
//    public function ($fromDate, $toDate, $type, $distid)
//    {
//        //enrollments
//        $nType = [];
//        if ($type == 'enrollments') {
...
//        }
//        $enrollments = \App\Order::getOrdersByTsaDateRange($fromDate, $toDate, $nType, $distid);
//        print_r($enrollments);
//        die;
////        $d = array();
...
//    }
    public function getFsbCommissionDataTable()
    {
        ...
        if ($req->volume_type == 'enrollments') {
            $nType = ['Enrollment', 'Upgrade'];
        } else if ($req->volume_type == 'subscriptions') {
            $nType = ['Membership', 'Boomerang', 'Pre-Paid Codes'];
        } else if ($req->volume_type == 'all') {
            $nType = ['Membership', 'Boomerang', 'Pre-Paid Codes', 'Enrollment', 'Upgrade'];
        }
        ...
        $query = DB::table('commission')
            ...
            ->where('products.id', '!=', \App\Product::ID_NCREASE_ISBO)
    ...
    public function getPearReportByUser($id = null)
    {
        ...
        return view('affiliate.reports.pear')->with($d);
    }
     ...
     * @return mixed
     */
    public function getPearReport($id = null)
    {
        $current = Carbon::now();
        ...
        $query = DB::select(sprintf("
            ...
            FROM get_personal_sponsor_qv('{$user->distid}', {$user->current_month_rank})
            JOIN users on users.id = user_id
            JOIN rank_definition on users.current_month_rank = rank_definition.rankval
            left join (SELECT COALESCE(SUM(o.orderqv), 0) as pqv, userid FROM users u JOIN orders o
              ON u.id = o.userid
    ...
    public function getHistoricalReport()
    {
        ...
        $currentMonth = utill::getUserCurrentDate()->startOfMonth();
        $currentWeek = utill::getUserCurrentDate()->startOfWeek();
        ...
        return view('affiliate.reports.historical')->with([
    ...
    public function entireOrganizationReport() {
        ...
        return view('affiliate.reports.entire_organization_report')->with($d);
    }

    public function weeklyBinaryReport($from = null, $to = null)
    {
        $d = array();
        $d['from'] = $from;
        $d['to'] = $to;
    ...
    public function weeklyBinaryReportDataTable(Request $request)
    {
        ...
        $lastDayOfTheWeek = date('Y-m-d', strtotime($firstDayOfTheWeek . ' + 7 days'));
        if ($req->from != "" && $req->to != "") {
            $firstDayOfTheWeek = date('Y-m-d', strtotime($req->from));
            $lastDayOfTheWeek = date('Y-m-d', strtotime($req->to));
        }
        ...
        $query = "select b.distid,b.firstname,b.lastname,b.username,b.created_dt,b.current_product_id,x.* from (
SELECT *, 'R' as direction FROM public.get_binary_orders(
  '$distid',
  'R',
  '$firstDayOfTheWeek',
...
UNION ALL
SELECT *, 'L' as direction  FROM public.get_binary_orders(
  '$distid',
  'L',
   '$firstDayOfTheWeek',
  '$lastDayOfTheWeek 23:59:59'
)) x, users b
join products p on b.current_product_id = p.id
where x.user_id = b.id $criteria
            order by date(b.created_dt) desc";
    ...
    public function getEnrolledInternDataTable($distid) {
...
//join binary_plan on users.id = binary_plan.user_id where tree.distid = users.distid;");
    ...
    public function getDistributorsByLevelDetailDataTable($level) {
//        $query = DB::select("select * from enrolment_tree('" . Auth::user()->distid . "') where level=" . $level);
        $query = DB::select("select created_dt,tree.*,binary_plan.direction from (select * from enrolment_tree('" . Auth::user()->distid . "'))tree, users
        join binary_plan on users.id = binary_plan.user_id where tree.distid = users.distid and tree.level = " . $level);
        return DataTables::of($query)->toJson();
    }

    public function getOrgDrillDownDataTable($distid) {
        $query = DB::select("select * from enrolment_tree('" . $distid . "') where distid <> '" . $distid . "'");
        return DataTables::of($query)->toJson();
    }

    public function invoice() {
        $orders = \App\Order::getByUser(Auth::user()->id);
        return view('affiliate.reports.invoice')->with(['orders' => $orders]);
    }


    public function viewInvoice($orderId) {
        $order = \App\Order::getUserOrder($orderId);
        if (!$order) {
        ...
        $compiled = view('affiliate.invoice.template')
            ->with([
                'user' => Auth::user(),
                'order' => $order,
                'order_items' => \App\OrderItem::where('orderid', $orderId)->get(),
                'address' => \App\Address::getBillingAddress(Auth::user()->id)
            ])->render();
    ...
    public function exportEnrollmentsByDate($sort_col, $asc_desc, $q = null) {
        ...
        $callback = function () use ($recs, $columns) {
            ...
            foreach ($recs as $rec) {
                fputcsv($file, array($rec->distid, $rec->firstname, $rec->lastname, $rec->username, $rec->created_dt,
    ...
    private function getUsersAndCount(Request $request)
    {
        $draw = $request->get('draw');
        $start = $request->get('start');
        $length = $request->get('length');
    ...
    public function losReport()
    {
        return view('admin.reports.los');
    }

    public function getLosReport()
    {
        request()->session()->put('distid', request()->distid);

        $distid = request()->distid;

        if (is_null($distid)) {
            return redirect()->back()->withErrors('error', 'A username or distid is required')->withInput();
        }
        ...
        if (!is_null($distid)) {
            $user = $user->where('distid', '=', strtoupper($distid))
                        ->orWhere('username', '=', strtolower($distid))
                        ->first();
        }

        if (!$user) {
            return redirect()->back()->with('error', 'No user was found')->withInput();
        }

        if (!$user->sponsorid) {
            return redirect()->back()->with('error', 'User finding, but he has no sponsor')->withInput();
        }

        $user->lifetime_rank = $user->rankHistory()->max('lifetime_rank');

        $userStack = new \SplStack();
        ...
        do {
            $user = User::query()->select($columns)->where('distid', '=', $user->sponsorid)->first();
            ...
            if ($user->lifetime_rank == 50 && $first['Sapphire'] == 'None') {
                $first['Sapphire'] = $user->distid;
            } else if ($user->lifetime_rank == 60 && $first['Ruby'] == 'None') {
                $first['Ruby'] = $user->distid;
            } else if ($user->lifetime_rank == 70 && $first['Emerald'] == 'None') {
                $first['Emerald'] = $user->distid;
            } else if ($user->lifetime_rank >= 80 && $first['Diamond'] == 'None') {
                $first['Diamond'] = $user->distid;
            }

            // Top user masteraccount
            if ($user->distid == 'TSA0002566') {
                break;
            }
        ...
        while (!$userStack->isEmpty()) {
            $response['users'][] = $userStack->pop();
        }

        $response['numSponsors'] = sizeof($response['users']) - 1;

        return view('admin.reports.los')->with('response', $response);
