...
class TMTPayment
{
    ...
    public function readTransaction($purchaseId)
    {
        $options = array(
            'url' => $this->url . "/wp-json/tmt/v2/transactions/" . $purchaseId,
            'method' => 'GET',
            'headers' => array(
                "Content-Type: application/json",
                "Authorization: Bearer " . $this->token
        ...
        $curlResponse = $this->getCurlResponse($options);
        if (isset($curlResponse['id'])) {
            $this->channel = new TMTChanel($curlResponse['channels'], null, $curlResponse['currencies']);
            $this->transactionId = $curlResponse['id'];
            $this->booking = new TMTBooking();
            $this->booking->setId($curlResponse['bookings'][0]['id']);
            $this->booking->setCurrencies($curlResponse['bookings'][0]['currencies']);
            $this->booking->setTotal($curlResponse['bookings'][0]['total']);
            $this->booking->setReference($curlResponse['bookings'][0]['reference']);
            $this->psp = $curlResponse['psp'];
            $this->linkedId = $curlResponse['linked_id'];
    ...
    {
        $options = array(
            'url' => $this->url . "/wp-json/tmt/v2/transactions/",
            'method' => 'POST',
            'postFields' => "{\n    \"channels\": " . $this->channel->getId() . ",\n    \"bookings\": [\n    \t{\n    \t\t\"id\": " . $this->booking->getId() . ",\n    \t\t\"currencies\": \"" . $this->booking->getCurrencies() . "\",\n    \t\t\"total\": " . $this->booking->getTotal() . "\n    \t}\n\t],\n    \"currencies\": \"" . $this->channel->getCurrencies() . "\",\n    \"total\": " . $this->booking->getTotal() . ",\n    \"psp\": \"" .$this->psp ."\",\n    \"transaction_types\": \"refund\",\n    \"payment_methods\": \"credit-card\",\n    \"linked_id\": " . $this->transactionId . "\n}",
            'headers' => array(
                "Content-Type: application/json",
                "Authorization: Bearer " . $this->token
            ),
        ...
        $curlResponse = $this->getCurlResponse($options);
        $error = ['error' => 1];
        if (isset($curlResponse['status']) and $curlResponse['content'] == "Succeeded!" ) {
            $error['error'] = 0 ;
            $error['authorization'] = $this->transactionId;
            $error['msg'] = 'Order refunded successfully';
            return $error;
        }
        return $error;
    }
